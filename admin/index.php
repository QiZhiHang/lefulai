...
if ($_REQUEST['act'] == '') {
	...
	//自己新加，会员购卡数量，昨天，前天，大前天，大大前天，金额返数量
	//前四天的数量，多余的不展示
	$today = strtotime(date('Y-m-d')); //今日凌晨的时间戳
	//今天
	$sql_0 = 'SELECT * FROM ' . $ecs->table('account_log') . ' WHERE change_type = 57 AND change_time > "' . $today . '" AND change_time <= "' . ($today + 24 * 3600) . '"';
	$sql_1 = 'SELECT sum("pay_points") FROM ' . $ecs->table('account_log') . ' WHERE change_type = 57 AND change_time > ' . ($today - 24 * 3600) . ' AND change_time <= ' . $today;
	$sql_2 = 'SELECT sum("pay_points") FROM ' . $ecs->table('account_log') . ' WHERE change_type = 57 AND change_time > ' . ($today - 2 * 24 * 3600) . ' AND change_time <= ' . ($today - 24 * 3600);
	$sql_3 = 'SELECT sum("pay_points") FROM ' . $ecs->table('account_log') . ' WHERE change_type = 57 AND change_time > ' . ($today - 3 * 24 * 3600) . ' AND change_time <= ' . ($today - 2 * 24 * 3600);
	$sql_4 = 'SELECT sum("pay_points") FROM ' . $ecs->table('account_log') . ' WHERE change_type = 57 AND change_time > ' . ($today - 4 * 24 * 3600) . ' AND change_time <= ' . ($today - 3 * 24 * 3600);

	$one = $db->GetOne($sql_0);
	$two = $db->GetOne($sql_1);
	$three = $db->GetOne($sql_2);
	$four = $db->GetOne($sql_3);
	$five = $db->GetOne($sql_4);

	$smarty->assign('tj_one', $one);
	$smarty->assign('tj_two', $two);
	$smarty->assign('tj_three', $three);
	$smarty->assign('tj_four', $four);
	$smarty->assign('tj_five', $five);

	$smarty->assign('t_one', date("Y-m-d"));
	$smarty->assign('t_two', date("Y-m-d", strtotime("-1 day")));
	$smarty->assign('t_three', date("Y-m-d", strtotime("-2 day")));
	$smarty->assign('t_four', date("Y-m-d", strtotime("-3 day")));
	$smarty->assign('t_five', date("Y-m-d", strtotime("-4 day")));
	...
	if (($data === false) || (API_TIME < date('Y-m-d H:i:s', time() - 43200))) {
		...
		$Http = new Http();
		$Http->doPost('http://ecshop.010xr.com/dsc_checkver.php', $httpData);
