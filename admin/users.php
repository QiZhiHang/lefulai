...
if ($_REQUEST['act'] == 'list') {
	...
	$is_shop = (empty($_POST['is_shop']) ? 0 : intval($_POST['is_shop']));
	$nick_name = (empty($_POST['nick_name']) ? '' : trim($_POST['nick_name']));
	$re_username = (empty($_POST['re_username']) ? '' : trim($_POST['re_username']));
	$paypwd = (empty($_POST['paypwd']) ? '' : trim($_POST['paypwd']));

	$users = &init_users();

	if ($is_shop == 1 && !empty($re_username)) {
		sys_msg('店铺成员不能填写推荐人！', 1);
	}
	if ($is_shop == 0 && empty($re_username)) {
	...
	if ($is_shop == 1) {
		...
		if (empty($re_username)) {
			sys_msg('普通注册推荐人不允许为空', 1);
		}
		$sql = 'SELECT user_id,belong_shop,is_shop FROM ' . $ecs->table('users') . ' WHERE user_name = \'' . $re_username . '\'';
		$re_info = $db->getRow($sql);
		if (empty($re_info)) {
			sys_msg('未找到推荐人', 1);
		}
	} else {
	...
	if (!$users->add_user($username, $password, $user_registerMode, $other)) {
	...
	if (!empty($paypwd)) {
		$salt = rand(1000, 9999);
		$str = ' ("' . $user_id_arr['user_id'] . '","' . $salt . '","' . md5(md5($paypwd) . $salt) . '",1) ';
		$sql = 'INSERT INTO ' . $ecs->table('users_paypwd') . ' (`user_id`, `ec_salt`,`pay_password`, `user_surplus`) VALUES' . $str;
		$db->query($sql);
	}
	...
	if ($flag == 1) {
		...
		$other['is_shop'] = 0;
		if ($re_info['is_shop'] == 1) {
			$other['belong_shop'] = $re_info['user_id'];
		} else {
			$other['belong_shop'] = $re_info['belong_shop'];
		}
		$other['parent_id'] = $re_info['user_id'];
	}
