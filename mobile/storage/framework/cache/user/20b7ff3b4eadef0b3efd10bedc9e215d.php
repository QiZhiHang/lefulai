...
<div class="con">
    <div class="goods-info user-order of-hidden ts-3"style="padding-top:4rem">
        <nav class="tab-title b-color-f of-hidden" style="position:fixed">
            <ul class="dis-box">
                <li class="box-flex <?php if(empty($status)) { ?>active<?php } ?>"><a status="0" >我的卡包列表</a></li>
            </ul>
        </nav>
                    ...
                    <% if order_list!=""%>
                        ...
                        <section class="flow-checkout-pro  m-top08">
                            ...
                            <div class="f-c-p-orderid padding-all m-top1px b-color-f">
                                ...
                                <h4 class="t-remark2">
                                    <label class="t-remark">卡号：</label><%order.order_sn%>
                            ...
                            <div class="padding-all user-orderlist-shop dis-box text-all-select">
                                ...
                                <ul class="flow-checkout-smallpic box-flex">
                                    ...
                                    <li><img class="product-list-img" src="/card_<%order.card_img1%>.jpg" /></li>
                                    <li><img class="product-list-img" src="/card_r.jpg" /></li>
                                </ul>
                                <span class="t-jiantou">
                                    <span class="f-c-a-count">已获取 <%order.total_num%>次</span><i class="iconfont icon-jiantou tf-180"></i></span>
                            </div>
														<div class="padding-all f-05 user-order-money b-color-f">本卡累计获得金额 合计：<em class="t-first"><%#order.total_fee%></em></div>
...
<nav class="commom-nav dis-box ts-5" id="ectouch-top">
    ...
    <script type="text/javascript">
        //订单列表
        var infinite = $('.store_info').infinite({url: "<?php echo url('user/account/my_card_list');?>", template: 'j-product', params: 'status=<?php echo $status; ?>'});
