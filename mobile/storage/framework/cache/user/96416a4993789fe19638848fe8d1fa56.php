		 ...
		 <?php if($consignee_list) { ?>      
			<div class="flow-consignee-list j-get-consignee-one select-three">
				...
				<section class="flow-checkout-adr m-top08">
					<div class="flow-have-adr padding-all">
						<div class="f-h-adr-title dis-box">
			                  <div class="box-flex"><label><?php echo ($consignee['consignee']); ?></label>
			                  </div>
			                  <div class="box-flex"><div class="fr"><span class="f-05"><?php echo ($consignee['mobile']); ?></span></div></div>
			             </div>
			             <p class="f-04  m-top04"><?php echo ($consignee['address']); ?></p>
					</div>
						...
						<div class="ect-select fl">
							<label class="dis-box <?php if($consignee['address_id'] == $address_id ) { ?>active<?php } ?>" onclick="adderss_user(<?php echo ($consignee['address_id']); ?>)">
								...
								<span class="t-first margin-lr">
									<a class="ml10 ftx-05 " href="javascript:void(0);"><?php if($consignee['address_id'] == $address_id ) { ?>默认地址<?php } else { ?>设为默认<?php } ?></a>
								</span>
							</label>
						</div>
                        <div class="fr">
	                        <a href="<?php echo url('index/addaddress',array('address_id'=>$consignee['address_id']));?>">
	                        	<i class="iconfont icon-bianji"></i>编辑
	                        </a>
	                        <a href="<?php echo url('index/drop',array('address_id'=>$consignee['address_id']));?>">
				...
				<div class="ect-button-more dis-box filter-btn">
					<a href="<?php echo url('index/addaddress');?>" class="btn-submit box-flex">新增收货人信息</a>
				</div>
			</div>
			<?php } else { ?>
			<div class="no-div-message">
				...
				<p>亲，还没有收货地址哦～！</p>
				<a href="<?php echo url('index/addaddress');?>" class="t-first">点击添加一个收货地址</a>
			</div>
			<?php } ?>	
		</div>
    <!--快捷导航-->
...
<nav class="commom-nav dis-box ts-5" id="ectouch-top">
    ...
    <div class="common-show"></div>  
       ...
       function adderss_user(address_id){
                $.get('<?php echo url('user/index/ajax_make_address ' );?>',{address_id:address_id},function(data){
