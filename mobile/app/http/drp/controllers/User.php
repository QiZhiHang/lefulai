		...
		if (empty($row['drp_is_separate'])) {
			...
			if ($up_oid) {
				foreach ($up_oid as $kk => $vv) {
					// $sql = "SELECT o.order_sn, o.drp_is_separate, o.user_id, SUMog(.drp_money) as drp_money\r\n                    FROM " . $GLOBALS['ecs']->table('order_info') . ' o' . ' RIGHT JOIN' . $GLOBALS['ecs']->table('order_goods') . ' og ON o.order_id = og.order_id' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON o.user_id = u.user_id' . ' WHERE o.order_id = \'' . $vv['order_id'] . '\' ';

					$sql = "SELECT o.order_sn, o.drp_is_separate, o.parent_id as user_id, SUM(og.drp_money) as drp_money\r\n                    FROM " . $GLOBALS['ecs']->table('order_info') . ' o' . ' RIGHT JOIN' . $GLOBALS['ecs']->table('order_goods') . ' og ON o.order_id = og.order_id' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON o.user_id = u.user_id' . ' WHERE o.order_id = \'' . $vv['order_id'] . '\' ';

					$row = $GLOBALS['db']->getRow($sql);
					file_put_contents('C:\Users\Administrator\Desktop\test10.txt', $sql, FILE_APPEND);
					...
					for ($i = 0; $i < $num; $i++) {
						...
						// $row = $GLOBALS['db']->getRow('SELECT o.parent_id as user_id,o.user_name FROM ' . $GLOBALS['ecs']->table('users') . ' o' . ' LEFT JOIN' . $GLOBALS['ecs']->table('drp_shop') . ' u ON o.user_id = u.user_id' . ' WHERE o.user_id = \'' . $row['user_id'] . '\'');

						$row = $GLOBALS['db']->getRow('SELECT o.parent_id as user_id,o.user_id as new_id,o.user_name FROM ' . $GLOBALS['ecs']->table('users') . ' o' . ' LEFT JOIN' . $GLOBALS['ecs']->table('users') . ' u ON o.parent_id = u.user_id' . ' WHERE o.user_id = \'' . $row['user_id'] . '\'');

						// if ((0 < $setmoney) && (0 < $row['user_id'])) {
						if ((0 < $setmoney) && (0 < $row['new_id'])) {
							$this->writeDrpLog($vv['order_id'], $row['new_id'], $row['user_name'], $setmoney, $setpoint, $i, $is, 0);
						}
					}
				}
			}
