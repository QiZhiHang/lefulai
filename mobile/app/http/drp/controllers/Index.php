...
//禁止倒卖 jinmengwangluo   QQ群256172550
	...
	public function actionPurchasePay() {
		...
		$price = $this->db->getOne($sql);
		$this->assign('price', price_format($price));
		$payment_list = get_online_payment_list(true);
	...
	public function actionBalancePay() {
		if (IS_POST) {
			...
			$log = dao('pay_log')->where(array('log_id' => $log_id, 'is_paid' => 0))->find();
			if (empty($log)) {
				show_message("未找到信息");
			}
			if ($log['order_amount'] > 0) {

			} else {

			}
			// $up_res = dao('pay_log')->where(array(array('log_id' => $log_id)))->save(array('is_paid' => 1));

			order_paid($log_id, 2);

			show_message("操作完成", '', url('drp/user/index', array('back_act' => urlencode($url))));
		}
	}
