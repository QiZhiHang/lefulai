...
class Index extends \app\http\base\controllers\Frontend {
	...
	public function actionIndex() {
		...
		$sql = 'SELECT COUNT(*) FROM {pre}cart  WHERE ' . $this->sess_id . 'AND is_gift = 0 AND rec_type = \'' . $flow_type . '\'';
	...
	public function actionDone() {
		...
		$sql = 'SELECT COUNT(*) FROM {pre}cart WHERE ' . $this->sess_id . 'AND is_gift = 0 AND rec_type = \'' . $flow_type . '\' AND rec_id ' . db_create_in($_SESSION['cart_value']) . ' ';
		...
		if (isset($affiliate['on']) && ($affiliate['on'] == 1) && ($affiliate['config']['separate_by'] == 1)) {
			...
			$sql = 'SELECT parent_id FROM ' . $this->ecs->table('cart') . ' WHERE ' . $this->sess_id;
			$get_parenet_id = $GLOBALS['db']->getOne($sql);

			$parent_id = $get_parenet_id ? $get_parenet_id : 0;
		...
		if (is_dir(APP_DRP_PATH)) {
			...
			if (isset($drp_affiliate['on']) && ($drp_affiliate['on'] == 1)) {
				...
				$sql = 'SELECT id FROM {pre}drp_shop ' . ' WHERE user_id = ' . $order['parent_id'] . ' AND audit = 1 AND status = 1';
