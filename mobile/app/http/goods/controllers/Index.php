...
class Index extends \app\http\base\controllers\Frontend {
	...
	public function actionIndex() {
		...
		if (intval($_GET['drp']) > 0) {
			$sql = 'SELECT user_id FROM {pre}drp_shop WHERE audit=1 AND status=1 AND id=' . intval($_GET['drp']);
			$drp_info = $this->db->getOne($sql);
			// $this->assign('drp_id', $drp_info);
		}
		$this->assign('drp_id', intval($drp_info) > 0 ? intval($drp_info) : 0);
