...
class Index extends \app\http\base\controllers\Frontend {
	...
	public function actionIndex() {
		...
		$leiji_info = dao('users')->where(array('user_id' => $this->user_id))->field('leiji_money,user_name,is_shop,belong_shop')->find();
		$this->assign('leiji_money', !empty($leiji_info['leiji_money']) ? floatval($leiji_info['leiji_money']) : 0.00);
		$this->assign('user_other_info', $leiji_info);
	...
	public function actionAffiliate() {
		...
		$url = url('/', '', true, true) . '?u=' . $this->user_id . '&toreg=1';
