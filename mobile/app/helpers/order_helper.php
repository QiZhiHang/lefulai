...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0, $store_id = 0) {
	...
	if (0 < $num) {
		$sql = 'SELECT goods_number FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND goods_id = \'' . $goods_id . '\' ' . ' AND parent_id = ' . $_parent_id . ' AND goods_attr = \'' . $goods_attr . '\' ' . ' AND extension_code <> \'package_buy\' ' . ' AND rec_type = \'' . CART_GENERAL_GOODS . '\' AND group_id=\'\' AND warehouse_id = \'' . $warehouse_id . '\' AND store_id = \'' . $store_id . '\'';
		$row = $GLOBALS['db']->getRow($sql);

		$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND parent_id != ' . $_parent_id;
		$my_flag = $GLOBALS['db']->getOne($sql);
		if (intval($my_flag) > 0) {
			$GLOBALS['err']->add('分销商品不允许混合购买,请分开进行购买！', 5);
			return false;
		}
