...
function register($username, $password, $email, $other = array()) {
	...
	if (intval($other['re_user_id']) <= 0) {
		$GLOBALS['err']->add('请填写正确的推荐人！');
	}
	$shop_info = dao('users')->where(array('user_id' => intval($other['re_user_id'])))->field('is_shop,belong_shop,user_id')->find();
	if (empty($shop_info)) {
		$GLOBALS['err']->add('请填写正确的推荐人！');
	}
	$shop_id = 0;
	if ($shop_info['is_shop'] == 1) {
		$shop_id = $shop_info['user_id'];
	} elseif (intval($shop_info['belong_shop']) > 0) {
		$shop_id = intval($shop_info['belong_shop']);
	}
	if ($shop_id <= 0) {
		$GLOBALS['err']->add('所属店铺信息错误');
	}
	if (!$GLOBALS['user']->add_user($username, $password, $email)) {
		...
		if (isset($affiliate['on']) && ($affiliate['on'] == 1)) {
			...
			if ($up_uid) {
				...
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ' , belong_shop =' . intval($shop_id) . ' WHERE user_id = ' . $_SESSION['user_id'];
		...
		$paypwd = !empty($other['paypwd']) ? $other['paypwd'] : '';
		if ($other) {
		...
		if (!empty($paypwd)) {
			$paypwd_data = array();
			$paypwd_data['user_id'] = $_SESSION['user_id'];
			$paypwd_data['ec_salt'] = rand(1000, 9999);
			$paypwd_data['pay_password'] = md5(md5($paypwd) . $paypwd_data['ec_salt']);
			$paypwd_data['user_surplus'] = 1;
			dao('users_paypwd')->add($paypwd_data);
		}
